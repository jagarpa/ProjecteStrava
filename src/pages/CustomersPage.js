...
class CustomersPage {
    ...
    async getCustomers() {
        try {
          const RESPONSE = await fetch("https://frontendapp-a41ad-default-rtdb.firebaseio.com/0.json ")
    ...
    renderCustomersPage() {
        ...
        for (const name in this.customers) {
          if (Object.hasOwnProperty.call(this.customers, name)) {
            const ELEMENT = this.customers[name];
            console.log(ELEMENT)
